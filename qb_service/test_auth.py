...
try:
    ...
    print("✓ QuickBooks client created and token refreshed\n")
except Exception as e:
    print(f"✗ Failed to create QuickBooks client: {e}\n")
    exit(1)
...
try:
    ...
    print(f"✓ Legal Name: {company_info.LegalName}")
    print(f"✓ Company successfully connected!\n")
except Exception as e:
...
try:
    ...
    if job_expenses:
        print(f"\n✓ Found 'Job Expenses' account: {job_expenses[0].Name} (ID: {job_expenses[0].Id})")
    else:
        print("\n! 'Job Expenses' account not found - you may need to create it or use a different name")
        ...
        for acc in expense_accounts[:10]:
